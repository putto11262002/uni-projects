...
public abstract class ImageAction extends AbstractAction {
     ...
     * via {@link setTarget}. Do not have to specify accelerator key, as it will be the same key as the mnemonic key.
     * </p>
     * 
    ...
    ImageAction(String name, ImageIcon icon, String desc, Integer mnemonic) {
        ...
        if (mnemonic != null) {
            ...
            // using the same key as the mnemonic key
            putValue(ACCELERATOR_KEY, KeyStroke.getKeyStroke(mnemonic, Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()));
        }
    }
     ...
     * To construct an ImageAction you provide the information needed to integrate it with the interface.
     * Note that the target is not specified per-action, but via the static member {@link target}
     * via {@link setTarget}. Allow specifying accelerator key.
     * </p>
     *
     * @param name The name of the action (ignored if null).
     * @param icon An icon to use to represent the action (ignored if null).
     ...
     * @param mnemonic A mnemonic key to use as a shortcut  (ignored if null).
     */
    ImageAction(String name, ImageIcon icon, String desc, Integer mnemonic, KeyStroke keyStroke) {
        super(name, icon);
        if (desc != null) {
            putValue(SHORT_DESCRIPTION, desc);
        }
